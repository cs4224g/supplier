    ...
    with conn:
        for line in sys.stdin:
            ...
            if transact == 'N':
                ...
                print(warehouse)
                print(quantity)
                run_transaction(conn, lambda conn: new_order_transaction(conn, int(instruct[2]), int(
                ...
                run_transaction(conn, lambda conn: delivery_transaction(
                    conn, instruct[1], instruct[2]))
            elif transact == 'O':
                run_transaction(conn, lambda conn: execute_t4(
                    conn, instruct[1], instruct[2], instruct[3]))
            elif transact == 'S':
                run_transaction(conn, lambda conn: execute_t5(
                    conn, instruct[1], instruct[2], instruct[3], instruct[4]))
            elif transact == 'I':
                run_transaction(conn, lambda conn: execute_t6(
                    conn, instruct[1], instruct[2], instruct[3]))
            elif transact == 'T':
                run_transaction(conn, lambda conn: execute_t7(conn))
            elif transact == 'R':
                run_transaction(conn, lambda conn: execute_t8(
                    conn, instruct[1], instruct[2], instruct[3]))
