...
def delivery_transaction(conn, W_ID, CARRIER_ID):
    with conn.cursor() as cur:
        # test
        cur.execute(
            "SELECT * FROM proj.warehouse WHERE W_ID = %s",
            str(W_ID)
        )
        print(cur.fetchmany())
        ...
        for order_in_district in orders_in_districts:
            ...
            cur.execute(
                "SELECT O_W_ID, O_D_ID, O_ID, O_CARRIER_ID FROM proj.orders WHERE O_W_ID = %s AND O_D_ID = %s AND O_ID = %s", (
            ...
            # test order line
            print("before update order line")
            cur.execute(
                "SELECT OL_W_ID, OL_D_ID, OL_O_ID, OL_NUMBER, OL_DELIVERY_D FROM proj.order_line WHERE OL_W_ID = %s AND OL_D_ID = %s AND OL_O_ID = %s", (
                    order_in_district[0], order_in_district[1], order_in_district[2])
            )
            print(cur.fetchall())
            ...
            # test after order line
            print("after update order line")
            cur.execute(
                "SELECT OL_W_ID, OL_D_ID, OL_O_ID, OL_NUMBER, OL_DELIVERY_D FROM proj.order_line WHERE OL_W_ID = %s AND OL_D_ID = %s AND OL_O_ID = %s", (
                    order_in_district[0],
                    order_in_district[1], order_in_district[2])
            )
            print(cur.fetchall())
            ...
            # test customer
            print("before update customer")
            cur.execute(
                "SELECT C_W_ID, C_D_ID, C_ID, C_BALANCE, C_DELIVERY_CNT FROM proj.customer WHERE C_W_ID = %s AND C_D_ID = %s AND C_ID = %s", (
                    order_in_district[0], order_in_district[1], order_in_district[3])
            )
            print(cur.fetchall())
